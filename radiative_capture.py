...
def photon_energy(data):
    ...
    pgrid.Ept = ((numpy.zeros(mat.mat2D.shape) + egrid.Ek).T +egrid.ET).T                       #photon energy grid
...
def charge_states(data):
    ...
    cgrid.mu = numpy.array([-mat.nu2D, mat.nu2D*1e-6, mat.nu2D])                                #mu is 3D matrix 
    cgrid.Ept = numpy.array([pgrid.Ept, pgrid.Ept, pgrid.Ept])                                  #Ept grid of the shape of mu
    cgrid.theta = numpy.array([pgrid.theta, pgrid.theta, pgrid.theta])                          #theta grid of the shape of mu
...
def broadening_function(data):
    ...
    bfunc.broadening = scipy.exp(-2 * hrf.SHR * (bfunc.fB+1)) * scipy.exp(hrf.SHR * inputs.Eph * phycon.eVJ / (phycon.kB * inputs.T)) * bfunc.bessel
...
def photoionization_cross_section(data):
    ...
    pion.PCS_C = (16 / 3) * phycon.alpha * phycon.a_br**2 * phycon.r_h * phycon.eVJ * (1 / derived.eta_r) * (phycon.m_e / inputs.M_eff) * (2 * scipy.pi ) * (numpy.sqrt(2 * inputs.M_eff) / phycon.hbar)**3     
    pion.Gamma = (gamma(cgrid.mu + 1))**2 / gamma(2 * cgrid.mu + 1)
    pion.PCS_vMu = (2**(2 * cgrid.mu)) * pion.Gamma * (mat.nu3D * derived.a_ebr)**3
    pion.PCS_vE = ((numpy.sqrt(mat.Ek3D * phycon.eVJ))**3 / (cgrid.Ept * phycon.eVJ)) * ((numpy.sin((cgrid.mu + 1) * numpy.arctan(numpy.sqrt(cgrid.theta))))**2 / (cgrid.theta * (1 + cgrid.theta)**(cgrid.mu + 1))) 

    pion.PCS_E = pion.PCS_C * pion.PCS_vMu * pion.PCS_vE 
    pion.PCS_E = numpy.ma.masked_less_equal(pion.PCS_E,0)                               #photoionization cross section before weighing or summation overEk
    
    pion.PCS = weighing_function.w_function_2(data,pion.PCS_E,2)                        #weighed by density of unoccupied states per volume
    pion.PCoeff = pion.PCS * inputs.v_th                                               #photoionization coefficient
    
    pion.sa = 4 * math.sqrt(scipy.pi * derived.r_eh * phycon.eVJ / (phycon.kB * inputs.T))          #sommerfiled factor
    
    pion.sigma_k_c = (16 / 3) * phycon.alpha * phycon.a_br**2 * phycon.r_h * phycon.eVJ * (1 / derived.eta_r) * (phycon.m_e / inputs.M_eff) * (2 * scipy.pi ) / (phycon.hbar * phycon.c / derived.eta_r)**3 
    pion.sigma_k_gamma = (gamma(cgrid.mu + 1))**2 / gamma(2 * cgrid.mu + 1)
    pion.sigma_k_mu = (2**(2 * cgrid.mu)) * pion.sigma_k_gamma * (mat.nu3D * derived.a_ebr)**3
    pion.sigma_k_E = (mat.Ek3D * phycon.eVJ * (cgrid.Ept * phycon.eVJ)**2 / (cgrid.Ept * phycon.eVJ)) * ((numpy.sin((cgrid.mu + 1) * numpy.arctan(numpy.sqrt(cgrid.theta))))**2 / (cgrid.theta * (1 + cgrid.theta)**(cgrid.mu + 1)))
    pion.sigma_k_Energy = pion.sigma_k_c * pion.sigma_k_mu * pion.sigma_k_E 
    pion.Ccoeff = weighing_function.w_function(data,pion.sigma_k_Energy,2) * inputs.v_th         #weighed by density of unoccupied states per volume
...
def radiative_capture_cross_section(data):
    ...
    rcapt.factor = (cgrid.Ept * phycon.eVJ)**2 / (2 * inputs.M_eff * (phycon.c / derived.eta_r)**2 * mat.Ek3D * phycon.eVJ) #DIMENSIONLESS
    rcapt.CCS_E = rcapt.factor * pion.PCS_E                                      
    rcapt.CCS_E = numpy.ma.masked_array(rcapt.CCS_E, pion.PCS_E.mask)     #capture cross section before weighing or summation overEk
    rcapt.CCS = weighing_function.w_function(data,rcapt.CCS_E,2)          #weighed by density of unoccupied states per volume
...
def trap_state_rc(data):
    ...
    if inputs.trap_state == 'don':
            tsr.r_sign = derived.sa * pion.Ccoeff[2,:][::-1]                 #electron capture coefficient from the CB by donar(initial charge state +ve)  
            tsr.r_sigp = pion.Ccoeff[1,:]                                    #hole capture coefficent from VB by donar(initial charge state neu)
            
    else:
            tsr.r_sign =  pion.Ccoeff[1,:][::-1]                             #electron capture coefficient from the CB by acceptor(initial charge state neu) 
            tsr.r_sigp =  derived.sa * pion.Ccoeff[0,:]                      #hole capture coefficent from the acceptor to VB(initial charge state -ve) 
