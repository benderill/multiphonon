...
def input_parameters(data,obj):
    ...
    inputs.Ec = obj.Eg
    inputs.Ev = 0
    inputs.epsilon_l = obj.epsilon_l * phycon.eps_0     #relative permittivity at low frequency in F/m
    inputs.epsilon_h = obj.epsilon_h * phycon.eps_0     #relative permittivity at high frequency in F/m
    ...
    inputs.sign = obj.sign * 1e-4                       #fixed electron capture cross section in m2 to be used in simple SRH
    inputs.sigp = obj.sigp *1e-4                        #fixed hole capture cross section in m2 to be used in simple SRH
...
def derived_parameters(data):
    ...
    derived.a_ebr = phycon.a_br * (inputs.epsilon_l / phycon.eps_0) / (inputs.M_eff / phycon.m_e)       #effective Bohr radius in meters
    derived.r_eh = phycon.Qe**2 / (8 * scipy.pi * inputs.epsilon_l * derived.a_ebr * phycon.eVJ)        #effective Rydberg energy in eV.Divide by e to get in eV. Value in meV=2.4
    derived.eta_r = scipy.sqrt(inputs.epsilon_h / phycon.eps_0)                                         #refractive index of the material
    derived.sa = 4 * scipy.sqrt(scipy.pi * derived.r_eh * phycon.eVJ / (phycon.kB * inputs.T))
