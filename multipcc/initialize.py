...
class Initialize():
    ...
    def set_input_parameters(self):
        ...
        self.data.inputs.M_eff *= self.data.physical_constants.m_e  # effective mass in kg
    ...
    def derived_parameters(self):
        ...
        '''
        self.data.derived.a_ebr = self.data.physical_constants.a_br * (self.data.inputs.epsilon_l / self.data.physical_constants.eps_0) / (
            self.data.inputs.M_eff / self.data.physical_constants.m_e)  # effective Bohr radius in meters
        ...
        self.data.derived.r_eh = self.data.physical_constants.Qe**2 / \
            (8 * sc.pi * self.data.inputs.epsilon_l *
             self.data.derived.a_ebr * self.data.physical_constants.eVJ)
        # refractive index of the material
        self.data.derived.eta_r = np.sqrt(
            self.data.inputs.epsilon_h / self.data.physical_constants.eps_0)
        self.data.derived.sa = 4 * \
            np.sqrt(sc.pi * self.data.derived.r_eh * self.data.physical_constants.eVJ /
    ...
    def energy_grids(self):
        ...
        self.data.energy_grids.ET = np.linspace(
            4*self.data.derived.r_eh, self.data.inputs.Eg - 4*self.data.derived.r_eh, 1000)
        ...
        self.data.energy_grids.nu = np.sqrt(
            self.data.derived.r_eh / self.data.energy_grids.deltaE)  # nu
        # the maximum final energy is 3KbT away
        self.data.energy_grids.Ekmax = 3 * self.data.physical_constants.kB * \
            self.data.inputs.T / self.data.physical_constants.eVJ
        ...
        self.data.energy_grids.k = np.sqrt(self.data.energy_grids.Ek * self.data.physical_constants.eVJ * 2 *
                                           self.data.inputs.M_eff / self.data.physical_constants.hbar**2)
    ...
    def matrices(self):
        ...
        self.data.matrix.nu2D = np.broadcast_to(
            self.data.energy_grids.nu, (ek.size, et.size)).T
